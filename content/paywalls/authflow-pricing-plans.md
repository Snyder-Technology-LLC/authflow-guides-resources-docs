---
id: "authflow-pricing-plans"
title: "Authflow Pricing Plans"
description: "Understanding Authflow's pricing tiers and choosing the right plan for you"
category: "paywalls"
tags: ["pricing", "plans", "billing", "fees"]
contextKeys: ["plans-billing"]
difficulty: "beginner"
estimatedTime: 4
displayOrder: 5
relatedDocs: ["pricing-strategies"]
---

# Authflow Pricing Plans

Authflow offers flexible pricing plans to suit creators at every stage. Here's what each plan includes and how to choose the right one for you.

## Plan Comparison

### Free Plan - $0/month

**Perfect for getting started**

| Feature | Included |
|---------|----------|
| Paywalls | 1 |
| Transaction Fee | 10% |
| Revenue Limit | Unlimited |
| Analytics | Basic |
| Support | Community |

**Best for:** Testing the platform, single GPT creators, getting started

---

### Starter Plan - $9/month

**For growing businesses** ⭐ *Most Popular*

| Feature | Included |
|---------|----------|
| Paywalls | 5 |
| Transaction Fee | 10% |
| Revenue Limit | Unlimited |
| Analytics | Advanced |
| Support | Email |

**Best for:** Multiple GPTs, serious creators, growing businesses

---

### Pro Plan - $49/month

**For power users**

| Feature | Included |
|---------|----------|
| Paywalls | 30 |
| Transaction Fee | 10% |
| Revenue Limit | Unlimited |
| Analytics | Premium |
| Support | Email + Phone |

**Best for:** Professional creators, agencies, multiple products

---

### Enterprise Plan - Starting at $299/month

**Tailored for scale and support**

| Feature | Included |
|---------|----------|
| Paywalls | Unlimited |
| Transaction Fee | Custom (negotiable) |
| Revenue Limit | Unlimited |
| Analytics | Custom |
| Support | Priority (Email, Phone, Dedicated) |
| Extras | Private beta access, roadmap input |

**Best for:** Large businesses, agencies, high-volume creators

## Understanding Fees

### Transaction Fee Structure

All plans (except Enterprise with custom fees) charge:

- **Authflow Fee**: 10% per transaction
- **Stripe Fee**: ~2.9% + $0.30 per transaction (paid directly to Stripe)

### Example Calculation

| Item | Amount |
|------|--------|
| User pays | $10.00 |
| Authflow fee (10%) | -$1.00 |
| Stripe fee (~3%) | -$0.59 |
| **Your net payout** | **~$8.41** |

### Revenue Sharing

- You keep everything after fees
- No revenue caps on any plan
- Automatic payouts to your Stripe account

## Choosing the Right Plan

### Start with Free

- Perfect for testing the platform
- See if Authflow works for your use case
- Create one paywall to get started
- Upgrade when you need more paywalls

### Upgrade to Starter ($9/month) when:

- You want to monetize multiple GPTs
- You need advanced analytics
- You want email support
- You're ready to scale

### Upgrade to Pro ($49/month) when:

- You have many GPTs or products
- You need phone support
- You want premium analytics
- You're running a serious business

### Consider Enterprise ($299+/month) when:

- You need unlimited paywalls
- You want custom transaction fees
- You need priority support
- You want to influence product development

## How to Change Your Plan

1. Go to **"Plans & Billing"** in your Authflow dashboard (under ACCOUNT)
2. View your current plan
3. Click the upgrade button for the plan you want
4. Complete the payment process
5. Your new plan takes effect immediately

## Frequently Asked Questions

**Can I downgrade my plan?**
Yes, you can downgrade at any time. Your current plan benefits remain until the end of your billing period.

**What happens if I exceed my paywall limit?**
You'll need to upgrade to a higher plan to create additional paywalls.

**Is there a contract or commitment?**
No, all plans are month-to-month. Cancel anytime.

**Do you offer annual billing?**
Contact sales for annual billing options with potential discounts.
