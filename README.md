<div align="center">

# Authflow.ai Documentation

![Authflow Header](https://firebasestorage.googleapis.com/v0/b/facesift.firebasestorage.app/o/user_uploads%2F83EXwNWgaKe1BHmyd9ISw3HCEfi2%2F1765257859893-Screenshot%202025-12-09%20002411.png?alt=media&token=9db96d0a-123e-471c-854a-4c5412f0341b)

**Monetize your custom GPTs directly inside ChatGPT — subscriptions, paywalls, and Stripe payouts in minutes.**

[Get Started](https://authflow.ai) · [Dashboard](https://app.authflow.ai) · [Integration Guide](#-start-here-gpt-integration)

</div>

---

## 🚀 Quick Start (10–15 minutes)

1. **Create an Authflow account** → [authflow.ai](https://authflow.ai)
2. **Connect Stripe** → Follow the onboarding wizard
3. **Create a paywall** → Set your price, trial period, and billing cycle
4. **Copy the GPT schema + instructions** → From your paywall details page
5. **Paste into your custom GPT** → In the Actions configuration
6. **Publish and start earning** → Share your GPT link

> 💡 *"If you can copy and paste, you can implement a paywall."*

---

## ⭐ Start Here: GPT Integration

This is the most important step — connecting Authflow to your custom GPT.

**👉 [Step-by-step GPT Integration Guide](content/integration/custom-gpt-overview.md)**

### Integration Checklist

- [ ] Paywall created in Authflow
- [ ] Schema pasted into GPT action
- [ ] API key configured (Authentication → Custom)
- [ ] Privacy policy added
- [ ] Top Context Instructions added at **top** of GPT instructions
- [ ] Tested with subscribed + unsubscribed email

---

## What is Authflow?

Authflow is the first ChatGPT-native paywall system for monetizing custom GPTs. It handles:

- **Authentication** — Verify subscribers via email
- **Payments** — Powered by Stripe (secure, global)
- **Subscriptions** — Recurring billing, free trials, one-time payments
- **Analytics** — Track revenue, subscribers, and conversions

### Who is this for?

- GPT creators who want to earn from their work
- Businesses offering AI-powered tools
- Educators, coaches, and content creators
- Anyone with a valuable custom GPT

---

## Features

| Feature | Description |
|---------|-------------|
| **Paywalls** | Protect GPT access with email verification |
| **Subscriptions** | Monthly, annual, or one-time billing |
| **Free Trials** | 1–14 day trials to convert users |
| **Stripe Integration** | Bank-grade security, global payments |
| **Analytics** | Revenue trends, conversion rates, churn |
| **Multiple Paywalls** | Different pricing for different GPTs |

---

## Documentation

### Getting Started

| Guide | Description |
|-------|-------------|
| [Introduction](content/getting-started/introduction.md) | What is Authflow and why use it |
| [Create Account](content/getting-started/create-account.md) | Sign up and access the dashboard |
| [Connect Stripe](content/getting-started/connect-stripe.md) | Link your Stripe account for payouts |
| [Complete Onboarding](content/getting-started/complete-onboarding.md) | Finish the setup checklist |
| [Dashboard Overview](content/getting-started/understanding-dashboard.md) | Navigate the Authflow dashboard |

### Integration (The Important Part)

| Guide | Description |
|-------|-------------|
| [Integration Overview](content/integration/custom-gpt-overview.md) | Complete integration walkthrough |
| [Paste the Schema](content/integration/gpt-schema-setup.md) | Copy/paste the OpenAPI schema |
| [Configure Authentication](content/integration/gpt-authentication.md) | Set up API key authentication |
| [Add Privacy Policy](content/integration/gpt-privacy-policy.md) | Required privacy policy for actions |
| [Top Context Instructions](content/integration/gpt-instructions.md) | Critical instructions placement |
| [Testing Your Integration](content/integration/testing-integration.md) | Verify everything works |
| [Multiple GPTs](content/integration/multiple-gpts.md) | Use one paywall across multiple GPTs |
| [Publishing Your GPT](content/integration/publishing-gpt.md) | Go live and share your GPT |

### Paywalls & Pricing

| Guide | Description |
|-------|-------------|
| [Create a Paywall](content/paywalls/create-paywall.md) | Step-by-step paywall creation |
| [Pricing Strategies](content/paywalls/pricing-strategies.md) | How to price your GPT |
| [Subscription Models](content/paywalls/subscription-models.md) | Monthly vs. annual billing |
| [Free Trials](content/paywalls/free-trials.md) | Attract users with trials |
| [Authflow Pricing Plans](content/paywalls/authflow-pricing-plans.md) | Free, Starter, Pro, Enterprise |

### Troubleshooting

| Issue | Solution |
|-------|----------|
| [Paywall Not Appearing](content/troubleshooting/paywall-not-appearing.md) | Instructions placement, schema errors |
| [Authentication Issues](content/troubleshooting/authentication-issues.md) | API key, header name problems |
| [Schema Errors](content/troubleshooting/schema-errors.md) | Red errors when pasting schema |
| [Character Limit](content/troubleshooting/character-limit.md) | 8,000 character instruction limit |

---

## Common Questions

<details>
<summary><strong>Is coding required?</strong></summary>

No traditional coding — copy/paste configuration only. If you can edit a GPT's instructions and add an action, you can use Authflow.

</details>

<details>
<summary><strong>How do I get paid?</strong></summary>

Payments go directly to your connected Stripe account. Stripe handles payouts according to your settings (typically weekly or monthly).

</details>

<details>
<summary><strong>What are the fees?</strong></summary>

See the [Authflow Pricing Plans](content/paywalls/authflow-pricing-plans.md) for current fees. All plans charge a transaction fee plus standard Stripe processing fees.

</details>

<details>
<summary><strong>Can I use one paywall for multiple GPTs?</strong></summary>

Yes! Copy the same schema into multiple GPTs. Users authenticate once and can access all GPTs sharing that paywall.

</details>

<details>
<summary><strong>What if I hit the 8,000 character limit?</strong></summary>

See [Character Limit Troubleshooting](content/troubleshooting/character-limit.md). Tips: prioritize Authflow instructions at the top, optimize your custom instructions, use the Knowledge Base for long content.

</details>

---

## Resources & Support

- **Website**: [authflow.ai](https://authflow.ai)
- **Dashboard**: [app.authflow.ai](https://app.authflow.ai)
- **Email Support**: support@authflow.ai
- **Stripe Guide**: Available in your dashboard under Support

---

## Next Steps

1. **[Create your Authflow account](https://authflow.ai)** if you haven't already
2. **[Create your first paywall](content/paywalls/create-paywall.md)**
3. **[Integrate it into your GPT](content/integration/custom-gpt-overview.md)**
4. **Publish, share, and start earning!**

---

<div align="center">

*Last updated: December 2024*

**[Get Started Now →](https://authflow.ai)**

</div>
